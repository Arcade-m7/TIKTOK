...
class Manager:
	...
	def __init__(self,file,encryption):
		self.__end_of_enc = b'<end_of_enc>' ; self.__enc = encryption ; self.__break = False
	...
	def get(self,chunk=MB):
		...
		while True:
			...
			if not data:
				...
				break
			elif self.__end_of_enc in (data:=self.__counter + data):
				index = data.find(self.__end_of_enc) ; self.__counter = b''
				if index+len(self.__end_of_enc) == data.find(self.__end_of_bytes):
					self.store(index+len(self.__end_of_enc)+len(self.__end_of_bytes)) ; self.__break = True ;return pickle.loads(self.__enc.decrypt(decompress(data[:index])))
				else:
